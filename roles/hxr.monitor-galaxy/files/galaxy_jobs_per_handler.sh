...
jobs_per_handler=$(/usr/bin/gxadmin csvquery q "select handler, state, count(state) from job where state in ('new', 'queued', 'running') and handler like '%handler_sn06_%' group by handler, state order by handler")
